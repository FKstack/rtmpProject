    ...
    : QWidget(parent)
{
    ...
    transitionAnimator_ = new GridTransitionAnimator(this);
...
void VideoGridWidget::setDisplayFrameRateRequest(
    const QString &requested,
    int effectiveFps
)
{
    displayFpsRequest_ = requested;
    displayTargetFps_ = std::clamp(effectiveFps, 1, 60);
    canvasHost_->setDisplayFrameRateRequest(
        displayFpsRequest_, displayTargetFps_
    );
}

void VideoGridWidget::setValidationLayoutEnabled(bool enabled)
{
    validationLayoutEnabled_ = enabled;
    for (VideoWidget *widget : videoWidgets_) {
        if (widget != nullptr) widget->setTitleOverlayEnabled(!enabled);
    }
}
...
{
    return MonitoringGridLayout::dimensionsForCount(
        widgetCount, kMaximumVideoWidgetCount, kMaximumGridDimension
    );
}
...
{
    return MonitoringGridLayout::calculate(
        availableSize, dimensions, videoChromeSize, baseMargins,
        spacing, videoAspect
    );
}
...
VideoWidget *VideoGridWidget::addVideoWidget(const QString &deviceName)
{
    ...
    QVector<GridTransition> transitions;
    transitions.reserve(animationItems.size());
    for (const WidgetAnimationItem &item : std::as_const(animationItems)) {
        GridTransition transition;
        transition.overlay = item.snapshot.data();
        transition.startGeometry = item.startGeometry;
        transition.endGeometry = item.endGeometry;
        transitions.push_back(std::move(transition));
    }
    transitionAnimator_->start(
        transitions, newWidgetSnapshot,
        [this, animationItems, guardedNewWidget] {
...
bool VideoGridWidget::swapVideoWidgets(int firstIndex, int secondIndex)
{
    ...
    QVector<GridTransition> transitions;
    GridTransition firstTransition;
    firstTransition.overlay = static_cast<QWidget *>(firstOverlay);
    firstTransition.startGeometry = firstGeometry;
    firstTransition.endGeometry = secondGeometry;
    transitions.push_back(std::move(firstTransition));
    GridTransition secondTransition;
    secondTransition.overlay = static_cast<QWidget *>(secondOverlay);
    secondTransition.startGeometry = secondGeometry;
    secondTransition.endGeometry = firstGeometry;
    transitions.push_back(std::move(secondTransition));
    transitionAnimator_->start(
        transitions,
        nullptr,
        [this, guardedFirstWidget, guardedSecondWidget,
...
bool VideoGridWidget::enterInCanvasFullscreen(VideoWidget *videoWidget)
{
    ...
    canvasHost_->setTargetFps(displayTargetFps_);
...
void VideoGridWidget::applyInCanvasFullscreenSnapshot()
{
    ...
    canvasHost_->setSnapshot(VideoGridSceneBuilder::buildFullscreen(
        size(), devicePixelRatioF(), item
    ));
}
...
void VideoGridWidget::refreshRenderSnapshot()
{
    ...
    QVector<VideoGridSceneItem> items;
    items.reserve(videoWidgets_.size());
    for (int index = 0; index < videoWidgets_.size(); ++index) {
        ...
        item.displayMode = videoWidget->displayMode();
        item.title = videoWidget->deviceName();
        item.status = videoWidget->statusText();
    ...
    canvasHost_->setSnapshot(VideoGridSceneBuilder::buildGrid(
        size(), devicePixelRatioF(), items, monitoringWallMode_
    ));
}
...
VideoWidget *VideoGridWidget::createVideoWidget(const QString &deviceName)
{
    ...
    videoWidget->setTitleOverlayEnabled(!validationLayoutEnabled_);
