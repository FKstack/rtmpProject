...
constexpr std::size_t kMaximumTimingSamples = 20'000;

void appendTimingSample(std::vector<qint64> &samples, qint64 value)
{
    if (value < 0) return;
    if (samples.size() >= kMaximumTimingSamples) {
        samples.erase(samples.begin(), samples.begin() + samples.size() / 4);
    }
    samples.push_back(value);
}

qint64 timingP95(std::vector<qint64> samples)
{
...
void VideoCanvasHost::setTargetFps(int fps)
{
    ...
    // Poll at twice the presentation target.  Polling at exactly the source
    // period (for example 33 ms for a nominal 30 FPS stream) can phase-lock
    // just before each decoded frame and only observe every second frame.  The
    // dirty flag still guarantees that this does not issue duplicate paints.
    scheduleTimer_->setInterval(
        std::max(1, qRound(500.0 / static_cast<double>(targetFps_)))
    );
}

void VideoCanvasHost::setDisplayFrameRateRequest(
    const QString &requested,
    int effectiveFps
)
{
    requestedDisplayFps_ = requested;
    setTargetFps(effectiveFps);
}
...
RenderRuntimeMetrics VideoCanvasHost::runtimeMetrics() const
{
    ...
    metrics.requestedDisplayFps = requestedDisplayFps_;
    metrics.effectiveDisplayFps = targetFps_;
    ...
    metrics.deadlineMisses = renderStatistics.deadlineMisses;
    ...
    metrics.paintCpuP95Us = timingP95(paintCpuSamples_);
    metrics.uploadCpuP95Us = timingP95(uploadCpuSamples_);
    metrics.gpuTimeP95Us = timingP95(gpuTimeSamples_);
...
void VideoCanvasHost::scheduleTick()
{
    ...
    if (paintPending_ && controller_->pendingDirty() != 0U) {
...
void VideoCanvasHost::onSurfacePainted()
{
    ...
    appendTimingSample(paintCpuSamples_, statistics_.lastPaintCpuUs);
    appendTimingSample(uploadCpuSamples_, statistics_.lastUploadCpuUs);
    appendTimingSample(gpuTimeSamples_, statistics_.lastGpuTimeUs);
