...
QColor colorForKind(UserMessageKind kind)
{
    switch (kind) {
    case UserMessageKind::Information:
        return QColor(QStringLiteral("#d5d9de"));
    case UserMessageKind::Success:
        return QColor(QStringLiteral("#7ee787"));
    case UserMessageKind::Warning:
        return QColor(QStringLiteral("#f2c66d"));
    case UserMessageKind::Error:
        return QColor(QStringLiteral("#ff7b72"));
    }
...
void LogPanel::appendVisibleMessage(const UserMessage &message)
{
    ...
    format.setForeground(colorForKind(message.kind));
