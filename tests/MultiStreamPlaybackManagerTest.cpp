...
void MultiStreamPlaybackManagerTest::audioSelectionIsExplicitAndExclusive()
{
    MultiStreamPlaybackManager manager;
    const StreamId first = manager.addStream(
        QStringLiteral("Camera 01"),
        QStringLiteral("rtmp://127.0.0.1:1/live/camera001")
    );
    const StreamId second = manager.addStream(
        QStringLiteral("Camera 02"),
        QStringLiteral("rtmp://127.0.0.1:1/live/camera002")
    );

    QCOMPARE(manager.selectedAudioStream(), kInvalidStreamId);
    QVERIFY(manager.isAudioMuted());
    QVERIFY(manager.selectAudioStream(first));
    QCOMPARE(manager.selectedAudioStream(), first);
    QVERIFY(!manager.isAudioMuted());
    QVERIFY(manager.selectAudioStream(second));
    QCOMPARE(manager.selectedAudioStream(), second);
    manager.setAudioMuted(true);
    QVERIFY(manager.isAudioMuted());
    manager.setAudioMuted(false);
    QVERIFY(!manager.isAudioMuted());
    QVERIFY(manager.removeStream(second));
    QCOMPARE(manager.selectedAudioStream(), kInvalidStreamId);
    QVERIFY(manager.isAudioMuted());
    QVERIFY(!manager.selectAudioStream(9999));
    manager.clearAudioSelection();
    manager.clearAudioSelection();
}
...
void MultiStreamPlaybackManagerTest::metricsFileIsAtomicAndOmitsUrls()
{
    ...
    QVERIFY(id != kInvalidStreamId);
    RuntimeMetricsReporter reporter(&manager);
    reporter.setRenderMetricsProvider([] {
        RenderRuntimeMetrics metrics;
        metrics.requestedDisplayFps = QStringLiteral("30");
        metrics.effectiveDisplayFps = 30;
        metrics.fallbackOccurred = true;
        metrics.fallbackReason = QStringLiteral("test-fallback");
        return metrics;
    });
    reporter.setOutputPath(path);
    QVERIFY(manager.startStream(id));
    QVERIFY(reporter.writeSnapshot());
    ...
    const QJsonObject displayFrameRate = document.object()
                                             .value(QStringLiteral("displayFrameRate"))
                                             .toObject();
    QVERIFY(!displayFrameRate.isEmpty());
    QVERIFY(displayFrameRate.value(QStringLiteral("fallbackOccurred")).toBool());
    QCOMPARE(displayFrameRate.value(QStringLiteral("fallbackReason")).toString(),
             QStringLiteral("test-fallback"));
    QVERIFY(document.object().value(QStringLiteral("renderStatistics")).isObject());
    const QJsonObject audio = document.object()
                                  .value(QStringLiteral("audio"))
                                  .toObject();
    QVERIFY(!audio.isEmpty());
    QCOMPARE(audio.value(QStringLiteral("state")).toString(),
             QStringLiteral("unavailable"));
    QCOMPARE(audio.value(QStringLiteral("streamId")).toString(),
             QStringLiteral("0"));
    QVERIFY(audio.contains(QStringLiteral("packetsReceived")));
    QVERIFY(audio.contains(QStringLiteral("decodedPackets")));
    QVERIFY(audio.contains(QStringLiteral("packetsDropped")));
    QVERIFY(audio.contains(QStringLiteral("underruns")));
    QVERIFY(audio.contains(QStringLiteral("underrunDurationMs")));
    QVERIFY(audio.contains(QStringLiteral("pcmBufferedMs")));
    QVERIFY(audio.contains(QStringLiteral("actualSinkBufferMs")));
    QVERIFY(!payload.contains("private-path"));
    QVERIFY(!payload.contains("rtmp://"));
    manager.stopAll();
}

void MultiStreamPlaybackManagerTest::emptyMetricsSnapshotPreservesSchemaV4Defaults()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString path = directory.filePath(QStringLiteral("metrics.json"));
    ...
    QCOMPARE(root.value(QStringLiteral("streamCount")).toInt(), 0);
    QVERIFY(root.value(QStringLiteral("streams")).toArray().isEmpty());
    QCOMPARE(root.value(QStringLiteral("renderer")).toObject()
                 .value(QStringLiteral("requestedBackend")).toString(),
             QStringLiteral("cpu"));
    QCOMPARE(root.value(QStringLiteral("renderer")).toObject()
                 .value(QStringLiteral("activeBackend")).toString(),
             QStringLiteral("unknown"));
    QCOMPARE(root.value(QStringLiteral("displayFrameRate")).toObject()
                 .value(QStringLiteral("requested")).toString(),
             QStringLiteral("auto"));
    QCOMPARE(root.value(QStringLiteral("displayFrameRate")).toObject()
                 .value(QStringLiteral("effective")).toInt(), 30);
    QVERIFY(root.value(QStringLiteral("displayFrameRate")).toObject()
                .value(QStringLiteral("achievedByStream")).toArray().isEmpty());
    QVERIFY(root.value(QStringLiteral("renderStatistics")).toObject()
                .contains(QStringLiteral("boundMailboxCount")));
