...
private slots:
    ...
    void mainWindowInstallsScrollableDeviceControlPanel();
    void dockLayoutPrioritizesSideControls();
    void connectionDialogValidatesInput();
    void dialogsExposeThemeRoles();
...
void VideoGridDynamicTest::mainWindowShowsEmptyConnectionPage()
{
    ...
    QCOMPARE(mainWindow.objectName(), QStringLiteral("mainWindow"));
    QCOMPARE(mainWindow.windowTitle(), QStringLiteral("RtmpMonitor 监控台"));
    ...
    auto *emptyCard = mainWindow.findChild<QFrame *>(
        QStringLiteral("emptyStateCard")
    );
    auto *brandLabel = mainWindow.findChild<QLabel *>(
        QStringLiteral("productBrandLabel")
    );
    QVERIFY(emptyButton != nullptr);
    QVERIFY(emptyButton->minimumHeight() >= 40);
    QVERIFY(emptyButton->isVisibleTo(&mainWindow));
    QVERIFY(emptyCard != nullptr);
    QVERIFY(brandLabel != nullptr);
    QCOMPARE(brandLabel->text(), QStringLiteral("RtmpMonitor 监控台"));
...
void VideoGridDynamicTest::mainWindowInstallsScrollableDeviceControlPanel()
{
    MainWindow mainWindow;
    auto *panel = new DeviceControlPanel(&mainWindow);
    mainWindow.installDeviceControlPanel(panel);

    auto *dock = mainWindow.findChild<QDockWidget *>(
        QStringLiteral("deviceControlDockWidget")
    );
    auto *scrollArea = mainWindow.findChild<QScrollArea *>(
        QStringLiteral("deviceControlScrollArea")
    );
    QVERIFY(dock != nullptr);
    QVERIFY(scrollArea != nullptr);
    QCOMPARE(dock->widget(), scrollArea);
    QCOMPARE(scrollArea->widget(), panel);
    QVERIFY(scrollArea->widgetResizable());
    QCOMPARE(scrollArea->horizontalScrollBarPolicy(), Qt::ScrollBarAlwaysOff);
}

void VideoGridDynamicTest::dockLayoutPrioritizesSideControls()
{
    ...
    auto *logDock = mainWindow.logDockWidget();
    auto *deviceDock = mainWindow.findChild<QDockWidget *>(
        QStringLiteral("deviceControlDockWidget")
    );
    auto *centralStack = mainWindow.findChild<QStackedWidget *>(
        QStringLiteral("centralStack")
    );
    QVERIFY(logDock != nullptr);
    QVERIFY(deviceDock != nullptr);
    QVERIFY(centralStack != nullptr);

    QCOMPARE(mainWindow.corner(Qt::TopLeftCorner), Qt::LeftDockWidgetArea);
    QCOMPARE(mainWindow.corner(Qt::BottomLeftCorner), Qt::LeftDockWidgetArea);
    QCOMPARE(mainWindow.corner(Qt::TopRightCorner), Qt::RightDockWidgetArea);
    QCOMPARE(mainWindow.corner(Qt::BottomRightCorner), Qt::RightDockWidgetArea);

    logDock->show();
    QTRY_VERIFY(logDock->isVisible());
    QTRY_VERIFY(deviceDock->geometry().top() <= logDock->geometry().top());
    QTRY_VERIFY(deviceDock->geometry().bottom() >= logDock->geometry().bottom());
    QTRY_VERIFY(logDock->geometry().right() < deviceDock->geometry().left());

    mainWindow.resizeDocks({logDock}, {300}, Qt::Vertical);
    QCoreApplication::processEvents();
    QVERIFY(deviceDock->geometry().top() <= logDock->geometry().top());
    QVERIFY(deviceDock->geometry().bottom() >= logDock->geometry().bottom());
    QVERIFY(logDock->geometry().right() < deviceDock->geometry().left());

    mainWindow.addDockWidget(Qt::LeftDockWidgetArea, deviceDock);
    QTRY_COMPARE(
        mainWindow.dockWidgetArea(deviceDock), Qt::LeftDockWidgetArea
    );
    QTRY_VERIFY(deviceDock->geometry().top() <= logDock->geometry().top());
    QTRY_VERIFY(deviceDock->geometry().bottom() >= logDock->geometry().bottom());
    QTRY_VERIFY(deviceDock->geometry().right() < logDock->geometry().left());

    const int centralWidthWithControl = centralStack->width();
    deviceDock->hide();
    QTRY_VERIFY(!deviceDock->isVisible());
    QTRY_VERIFY(centralStack->width() > centralWidthWithControl);
}
...
void VideoGridDynamicTest::dialogsExposeThemeRoles()
{
    ConnectionDialog connectionDialog;
    QCOMPARE(connectionDialog.objectName(), QStringLiteral("connectionDialog"));
    auto *connectButton = connectionDialog.findChild<QPushButton *>(
        QStringLiteral("addAndConnectButton")
    );
    QVERIFY(connectButton != nullptr);
    QCOMPARE(connectButton->property("styleRole").toString(),
             QStringLiteral("primary"));
    QVERIFY(connectButton->minimumHeight() >= 40);

    SavedStreamsDialog savedStreamsDialog;
    auto *removeButton = savedStreamsDialog.findChild<QPushButton *>(
        QStringLiteral("removeSavedStreamButton")
    );
    auto *savedConnectButton = savedStreamsDialog.findChild<QPushButton *>(
        QStringLiteral("connectSavedStreamButton")
    );
    QVERIFY(removeButton != nullptr);
    QVERIFY(savedConnectButton != nullptr);
    QCOMPARE(removeButton->property("styleRole").toString(),
             QStringLiteral("danger"));
    QCOMPARE(savedConnectButton->property("styleRole").toString(),
             QStringLiteral("primary"));

    SavedStreamEditorDialog editorDialog;
    auto *saveProfileButton = editorDialog.findChild<QPushButton *>(
        QStringLiteral("saveStreamProfileButton")
    );
    QVERIFY(saveProfileButton != nullptr);
    QCOMPARE(saveProfileButton->property("styleRole").toString(),
             QStringLiteral("primary"));

    MqttSettingsDialog mqttDialog;
    mqttDialog.setTestResult(QStringLiteral("连接成功"), false);
    auto *testResult = mqttDialog.findChild<QLabel *>(
        QStringLiteral("mqttTestResult")
    );
    QVERIFY(testResult != nullptr);
    QCOMPARE(testResult->property("severity").toString(),
             QStringLiteral("success"));
    mqttDialog.setTestResult(QStringLiteral("连接失败"), true);
    QCOMPARE(testResult->property("severity").toString(),
...
void VideoGridDynamicTest::monitoringWallRoundTripRestoresWindowChrome()
{
    ...
    QSignalSpy fullscreenTransitionSpy(
        &mainWindow, &MainWindow::fullscreenTransitionStarted);

    action->trigger();
    QCOMPARE(fullscreenTransitionSpy.count(), 1);
    ...
    QTRY_COMPARE(fullscreenTransitionSpy.count(), 2);
    ...
    QTRY_COMPARE(fullscreenTransitionSpy.count(), 3);
    ...
    QTRY_COMPARE(fullscreenTransitionSpy.count(), 4);
    QTRY_VERIFY(!mainWindow.isMonitoringWallMode());
    QTRY_VERIFY(!mainWindow.isFullScreen());
    QVERIFY(!action->isChecked());
    QVERIFY(mainWindow.menuBar()->isVisible());
