...
> `<WSL-distro>` 中，Windows 通过 `http://127.0.0.1:1933`
...
| OpenViking Server | WSL2 | systemd 服务、doctor、journal、服务端 CLI | `wsl.exe -d <WSL-distro>` |
...
```text
<workspace>\rtmpProject\.kimi-code\mcp.json
```
