...
> 日期：2026-08-15
> 最后更新：2026-08-16
> 状态：产品与架构联合接受；Phase 1、Phase 2A、模块三与 Phase 4 PoC 已实现并通过自动化门禁，现场验收待执行
> 风险：本文档变更为 R0；控制策略、事件、证据和 SRS DVR PoC 均按 R2 评审
> 试点：单车、单桌面操作者、本地离线优先
...
- 另建 DVR PoC 配置模板，只有用户显式渲染并运行时才启用 `dvr` 和 `http_hooks.on_dvr`。
...
- 对 callback file 执行规范路径解析，并验证最终路径位于配置的 DVR 根目录内；拒绝 `..`、符号链接
  越界、非普通文件和不存在文件。
- 记录流标识、可选开始时间/持续时间、相对路径、大小和 mtime 纳秒，不保存完整 RTMP URL、token、
  客户端 IP、SRS cwd 或绝对 DVR 路径。
- 不读取分段内容，不计算或比较 SHA-256 或其他内容哈希；收据固定声明
  `contentHashVerification=not_performed`，因此不提供文件未被修改或防篡改保证。
- 以“DVR 根内相对路径 + 大小 + mtime 纳秒”为幂等键，重复 callback 不重复登记。该键只用于本地
  重复回调抑制，不是内容身份或完整性证明。
- 收据先写 spool 临时目录再原子改名发布；本轮 Qt/EvidenceService 不读取收据，未来是否消费必须
  在产品化评审后另行设计。
- 监听只绑定回环，失败不影响 SRS 推拉流；适配器退出时不终止 SRS。

### 8.3 PoC 门禁

已在固定 SRS 6.0.184 上验证正常分段、关键帧边界、断流尾段、重复 callback、乱序 callback、路径
穿越/符号链接越界、低磁盘、SRS 重启、适配器重启、适配器离线、应用未运行和原始配置 DVR 关闭。
PoC 可独立运行不等于产品能力完成：当前产品仍只承诺截图证据；事件与录像关联、时间线回放、录像
导出、锁定保留、服务化安装和 ARM 现场部署不得进入已完成功能列表。
...
> 当前状态：已于 2026-08-16 按 R2 实现可运行 PoC；不进入 Windows 发布包，也未接入 Qt、事件或
> EvidenceService。

- 新增默认不被加载的配置模板、仅回环监听的 Python 标准库适配器、schema v1 原子收据和独立
  WSL/Linux 验证器，不改变默认 SRS 配置及 Qt 主进程。
- Python 单元测试 14/14 和 SRS 6.0.184 端到端矩阵通过；确认适配器故障不阻断推拉流，原始
  `srs-minimal.conf` 不产生录像或收据副作用。
- 不执行内容哈希，当前 PoC 不具备防篡改保证。进入事件前后录像实现前仍需单独完成产品化评审。
...
| DVR PoC | 默认关闭；适配器故障不影响推拉流；重复收据不重复登记；分段可读取、时长有界且从关键帧开始 |

## 12. 架构影响

- **风险等级**：本文档交付 R0；Phase 1、Phase 2A、模块三和 SRS DVR PoC 均按 R2 完成。
...
- **契约与生命周期**：MQTT、心跳、RTMP、CLI 和现有持久化 schema 不变；新状态在 Qt owner 线程，
  证据 I/O 有界且可停止；SRS 拥有录像，独立 callback 进程拥有收据 spool，Qt 不拥有二者。
- **当前验证**：Phase 1、Phase 2A、模块三与 Phase 4 PoC 已按 R2 实现；当前 Windows Debug CTest 为 36/36，
  Windows Release、ARM64 RASTER/GLES3 全目标构建、AArch64 ELF/动态依赖审计和 QEMU 逻辑测试
  通过；模块四 Python 单测 14/14 与固定 SRS 6.0.184 端到端故障矩阵通过。真实车辆动作、ARM 真机
  显示、ARM DVR 磁盘资格和现场证据流程仍待人工验收。
...
联合接受后 Phase 1、Phase 2A、模块三与 Phase 4 PoC 已按 R2 实现并通过上述自动化门禁；这仍不
表示车辆已执行命令、证据或 DVR 文件未被修改，也不表示事件录像产品能力或任何现场验收已经完成。
