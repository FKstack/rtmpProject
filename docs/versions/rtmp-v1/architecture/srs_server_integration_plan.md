...
- `docs/versions/rtmp-v1/guides/build-and-testing/rtmp_chain_verification.md`：增加 SRS 入口，不删除 nginx 历史。
...
- `docs/versions/rtmp-v1/guides/build-and-testing/cross_platform_build.md`：增加 SRS 部署边界和
  “交叉构建不等于实机通过”。
...
- 无新的 C++ 文件。
- 若摄像头厂商需要专门说明，只新增脱敏的
  `docs/versions/rtmp-v1/guides/build-and-testing/camera_to_srs.md`，不得保存鉴权 URL。
...
- 故障恢复复验以现有 SRS 生命周期/链路脚本和
  `docs/versions/rtmp-v1/guides/build-and-testing/srs_failure_recovery.md` 的步骤为准；如未来需要
