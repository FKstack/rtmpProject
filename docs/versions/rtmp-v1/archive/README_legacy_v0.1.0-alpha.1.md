# RtmpMonitor 旧版 README（历史归档）

> 这是提交 `1729da1` 时根 README 的整理归档，不再作为当前构建或测试入口。
> 其中的 Week 记录和性能数字用于保留历史背景；涉及已退役脚本的操作请参阅
> [遗留脚本索引](legacy_test_scripts.md)。当前用法以仓库根目录的新 README 和
> [现行构建指南](../guides/build-and-testing/cross_platform_build.md)为准。

> 原始、未经整理的版本可运行 `git show 1729da1:README.md` 查看。
...
完整路线见[项目规划](../../../roadmap/project_plan.md)，四路版本的历史基线见
[首批四路播放](../weeks/week4/week4_multi_stream_playback.md)，当前实现和验收方法见
[16 路架构与验收](../weeks/week4/week4_sixteen_stream_validation.md)，本次变更和实测记录见
[Week 6 产品级 OpenGL 验证总览](../weeks/week6/week6_opengl_environment_and_validation.md)，
生产实现见[产品级视频渲染框架](../architecture/video_rendering_framework.md)。旧版源码 ZIP
交接链路已经退役，当前以 Git 分支和精确提交为源码交接基准。

当前发布标识为 `RtmpMonitor v0.1.0-alpha.1`：Windows 为 **Development Preview**，
...
| SRS Server 接入 | Windows+WSL2 最小链路通过 | 2026-08-09 独立复验了 VS Preset 构建、SRS 6.0.184、1935/回环 1985、Windows/WSL 双侧推拉流和安全停止；ARM 实机、真实摄像头和网页现场结果仍 `[需要验证]` |

> 2026-08-05 的四组 600 秒结果验证了产品 Renderer，但早于 2026-08-08 的最终
> 监控墙布局。当前代码后续已完成 Windows Debug CTest 18/18，但该轮未设置
> `RTMP_MONITOR_TEST_URL`；当前布局的性能证据仍只有 120 秒快速对照。快速对照中
> OpenGL CPU 降低 91.66%、显示 14.914 FPS，但 latest frame age P95 为 52 ms，
> 超过 51.7 ms 门槛 0.3 ms，因此不能把旧长测重新表述成当前布局的正式认证或新的
> 真实流复验。
...
`x64-windows` 提供的 FFmpeg 8.1.2 LGPL 开发库。首次 clone 后使用统一入口检查
VS/Qt、安装固定版本 FFmpeg、生成本机 Preset，并完成 Debug 构建和 CTest：
...
脚本不会自动安装 Visual Studio 或 Qt；这两项缺失时会给出明确诊断。`ToolRoot`、
下载、二进制缓存、TEMP 和构建目录均可通过参数放到空间充足的磁盘。底层
`setup_ffmpeg_windows_dev.ps1` 固定 vcpkg commit 和 FFmpeg 8.1.2，仅由统一入口或
明确的高级维护操作调用。开发启动仍以 Visual Studio 为准：

1. 日常开发选择 `Qt-Debug`，发布候选验证选择 `Qt-Release`；两者都必须使用
   Visual Studio/MSVC Qt Kit；
...
`CMakeUserPresets.json` 保存 Qt/vcpkg 的个人路径且不提交。也可以复制受跟踪的
`CMakeUserPresets.example.json` 后手工替换两个占位符。统一入口会在隔离子进程中
初始化 MSVC，并过滤 MinGW/MSYS 路径；不能混用 MinGW Qt 和 MSVC 产物，也不要把
双击未部署 Qt DLL 的 EXE 当作开发期标准启动方式。
...
# check 报告环境不完整时才执行；只有 install 需要 root。
sudo bash scripts/setup_linux_arm64_dev.sh --action install

bash scripts/setup_linux_arm64_dev.sh \
  --action all --render-mode both
```

`all` 会分别构建 RASTER/GLES3、检查 AArch64 ELF/依赖并运行可在 QEMU 中可靠执行的
纯逻辑测试。代理只接受显式 `--proxy-url`，不会假定维护者本机端口。交叉构建只能
证明二进制和依赖 ABI 正确，不能代替目标盒子的 QPA、窗口系统、VPU、网络、持续
播放和性能测试。
...
结果表见[16 路架构与验收](../weeks/week4/week4_sixteen_stream_validation.md)。
...
新手从
[SRS 新手完全指南](../guides/build-and-testing/srs_beginner_guide.md)入手；
完整方案、ARM 部署与逐 Phase 验收见
[SRS Server 接入实施方案](../architecture/srs_server_integration_plan.md)和
[RTMP 链路验证 §15](../guides/build-and-testing/rtmp_chain_verification.md)；
...
- [文档分类索引](../README.md)
- [项目规划](../../../roadmap/project_plan.md)
- [Week 4～5 代码框架、维护与深入学习指南](../guides/architecture/week4_week5_architecture_guide.md)
- [Week 4 多路媒体与并发深度学习](../guides/architecture/week4_media_concurrency_deep_dive.md)
- [用户事件、系统日志与审计日志架构](../guides/architecture/logging_architecture.md)
- [Week 4 新对话公开交接](../weeks/week4/week4_conversation_handoff.md)
- [Week 5 设备状态、日志与重连](../weeks/week5/week5_device_status_and_logging.md)
- [Week 6 产品级 OpenGL 渲染与验证总览](../weeks/week6/week6_opengl_environment_and_validation.md)
- [Week 6 产品视频渲染框架教学篇](../weeks/week6/week6_product_rendering_framework_tutorial.md)
- [Week 6 CPU/OpenGL 自动化对照测试实战篇](../weeks/week6/week6_renderer_performance_test_guide.md)
- [16 路动态连接、解码架构与验收](../weeks/week4/week4_sixteen_stream_validation.md)
- [Week 4 模块变更与测试操作记录](../weeks/week4/week4_release_test_and_module_changes.md)
- [四路历史基线](../weeks/week4/week4_multi_stream_playback.md)
- [Windows x64 与 Linux ARM64 构建](../guides/build-and-testing/cross_platform_build.md)
...
- [第三周播放器](../weeks/week3/week3_ffmpeg_player.md)
- [桌面端到端延迟基线](../weeks/week3/week3_desktop_latency_test.md)
...
[视频渲染框架](../architecture/video_rendering_framework.md)及
