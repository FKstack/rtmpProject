...
> `docs/versions/rtmp-v1/weeks/week7/week7_srs_server_integration.md`。第 2.6 节网页预览是按
> SRS 6.0.184 官方配置与固定 tag 源码核对后新增的可选教程；当前仓库默认
> 未启用 8080，现场运行结果明确标为 `[需要验证]`。

> 当前发布状态：RtmpMonitor `v0.1.0-alpha.1`；Windows Development Preview；
> ARM Linux Engineering Preview。Windows+WSL2 最小 SRS 链路已独立复验；
> 4/16 路与 600 秒结果属于历史测试证据；ARM 真机、真实摄像头和网页预览现场
> 结果均为 `[需要验证]`。
...
- 历史上的 nginx-rtmp 本机脚本已经退役；历史背景见
  [`docs/versions/rtmp-v1/archive/legacy_test_scripts.md`](../../archive/legacy_test_scripts.md)，新工作一律用 SRS。
...
```powershell
$env:RTMP_MONITOR_WSL_DISTRO = "<WSL-distro>"
...
- 通过 `RTMP_MONITOR_WSL_DISTRO` 或 `-Distro` 指定的 WSL2 发行版是否存在；
...
先运行 `wsl.exe --list --quiet` 查询名称，再进入自己的 WSL 发行版：

```powershell
wsl.exe -d "<WSL-distro>"
```
...
**真实 1080p H.264 文件原质量推流**：先确认文件确实是 H.264、1920×1080，
并记录帧率和源文件声明的码率。以下命令假设接收方把自己的测试文件放在当前目录
并命名为 `input-1080p.mp4`；该文件不放入仓库。

WSL/Bash：

```bash
input_file="$PWD/input-1080p.mp4"
...
```powershell
$InputFile = Join-Path $PWD 'input-1080p.mp4'
...
`-c:v copy` 不重新编码，只保留输入文件已有的压缩质量；它不会把模糊、低码率或
低分辨率视频增强为高清。只有输入已经是与 FLV/RTMP 兼容的 H.264 且时间戳正常时
才使用 copy；如果 FFmpeg 报封装/时间戳不兼容，改用下面的转码路径。
...
```bash
ffprobe -v error \
  -show_entries stream=index,codec_type,codec_name,width,height,avg_frame_rate,bit_rate:format=bit_rate \
  -of json \
  rtmp://127.0.0.1:1935/live/camera01
...
ffmpeg -i rtmp://127.0.0.1:1935/live/camera01 \
  -frames:v 45 -f null -
```
...
直播输入的容器或 stream `bit_rate` 可能显示为 `N/A`/空值，这不等于零码率。此时
以推流端设置、SRS 统计或固定时间窗口的接收字节量为准；分辨率、帧率和编码仍以
上述 `ffprobe` 结果为准。
...
开发期以 **Visual Studio F5** 为唯一首选入口：日常调试选择 `Qt-Debug`，发布候选
复验选择 `Qt-Release`；两者都必须使用 MSVC Qt 和 vcpkg `x64-windows` FFmpeg。

1. 选择 `Qt-Debug` 或 `Qt-Release` CMake Preset；
...
先在仓库根目录的 Windows PowerShell 启动 SRS：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\srs\srs_dev_wsl.ps1 -Action Start
```

再在 WSL/Bash 推送测试流；下面只展示 camera01，camera02～04 使用不同流名分别启动：
...
复制示例并增加 camera01～04 段后，在 Visual Studio 中使用 `Qt-Debug`/F5 启动；
以下 Windows PowerShell 命令只用于诊断后备：
...
**Q12：怎样验证真实音频链路和延迟？**
使用 `scripts/audio/qualify_mp4_audio.ps1`。脚本只下载中国境内阿里云官方正常 MP4 到
`out/qualification/`，校验 H.264/AAC 后实时推送到本脚本管理的 SRS，再由正式
`AudioPlaybackEngine` 拉流、解码并写入 QAudioSink。每轮必须从连续媒体开头开始，不在实时
发布时循环短文件；否则文件循环边界会制造与摄像头无关的假欠载。2026-08-15 三轮 320 秒和
一轮 600 秒均通过，600 秒 P50/P95/最大为 89.561/111.632/125.396 ms，欠载 0。该数字不包含
扬声器实际出声，声学延迟仍需回环设备。
...
- 单向音频架构与资格结果：[../../architecture/low_latency_audio_stream.md](../../architecture/low_latency_audio_stream.md)
- 总体方案与 `[需要验证]` 清单：`docs/versions/rtmp-v1/architecture/srs_server_integration_plan.md`
