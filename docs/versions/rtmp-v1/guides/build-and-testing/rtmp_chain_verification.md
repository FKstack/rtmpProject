...
> 文档分类：历史构建与验证记录。
>
> **注意：** 本文第 1～14 节描述的 `scripts/verify_rtmp_chain.ps1` 已从当前分支退役，
> 其中命令仅用于理解早期 nginx-rtmp 流程，请勿直接执行。当前开发请使用
> [SRS 新手指南](srs_beginner_guide.md)和本文第 15 节；删除原因见
> [遗留脚本索引](../../archive/legacy_test_scripts.md)。
...
```text
<workspace>\rtmpProject\scripts\verify_rtmp_chain.ps1
```
...
```text
脚本目录：<workspace>\rtmpProject\scripts
脚本目录的父目录：<workspace>\rtmpProject
默认测试视频：<workspace>\rtmpProject\testdata\test.mp4
```
...
测试视频：
<workspace>\rtmpProject\testdata\test.mp4
...
.\scripts\verify_rtmp_chain.ps1 `
    -Action Check `
    -NginxRoot "<alternate-nginx-root>"
```
...
.\scripts\verify_rtmp_chain.ps1 `
    -Action All `
    -InputFile "<media-root>\camera_test.mp4"
```
...
```text
scripts/verify_rtmp_chain.ps1
docs/versions/rtmp-v1/guides/build-and-testing/rtmp_chain_verification.md
```
...
> SRS 接入是 `docs/versions/rtmp-v1/architecture/srs_server_integration_plan.md` 的 Phase 1 产物。本节给出
> 当前 SRS 链路入口；上方第 1～14 节只保留为 nginx-rtmp 历史说明，
> `scripts/verify_rtmp_chain.ps1` 已从当前分支删除。
...
当前原则：新环境只使用 SRS 脚本。需要研究旧 nginx 脚本时从 Git 历史读取，
不要把维护者的本机路径和依赖布局直接恢复到公共入口。

### 15.2 前置条件

- WSL2 发行版 `<WSL-distro>`，已按方案文档第 6 节在
  `$HOME/opt/srs-6.0.184` 完成源码构建与安装（固定 tag `v6.0-r0`）。
- 无 root 环境时，构建依赖 tclsh 可用户态解包到 `$HOME/local`（见
  `docs/versions/rtmp-v1/weeks/week7` 的 Phase 1 记录）。
- Windows 端 `ffmpeg/ffprobe/ffplay` 在 PATH 中（用于 Windows 侧拉流验证）。

### 15.3 日常使用

先查询并指定接收方自己的 WSL 发行版：
