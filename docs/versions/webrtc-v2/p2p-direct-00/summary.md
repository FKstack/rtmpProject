...
> 2026-09-02：ADR-048 已覆盖本摘要对现有公网 Broker 的“仅测试”定位；当前团队产品允许使用其正常
> MQTT 客户端数据面。本文其余内容作为 DIRECT-00 历史证据保留。
...
依赖图和分层门禁，并为隔离产品 Broker 候选保留可选资格测试。当时现有远程明文服务仅允许兼容性观察，不是
产品候选，也不得成为源码或配置默认值。
