...
param(
    ...
    [string]$Distro = 'Ubuntu-22.04-New',
...
function Invoke-Wsl {
    # Runs a bash script inside the distro via stdin (bash -s), avoiding every
    # wsl.exe/PowerShell argument-quoting layer ($HOME etc. expand in bash).
...
function Invoke-Check {
    ...
    if ($version -match 'SRS_BINARY_MISSING') {
        Write-Step "SRS binary: MISSING at $SrsHome/objs/srs (build it first, see docs/srs_server_integration_plan.md section 6)"
    } else {
