...
readonly smoke_directory="${ARM64_BUILD_ROOT:-${project_root}/out/ffmpeg-smoke-arm64}"
readonly smoke_binary="${smoke_directory}/ffmpeg_environment_smoke"

export PKG_CONFIG_SYSROOT_DIR="${sysroot}"
export PKG_CONFIG_LIBDIR="${pkgconfig_dir}"

for package_name in libavformat libavcodec libavutil libswscale libswresample; do
    pkg-config --exists "${package_name}"
    printf '%s ABI version: %s\n' \
        "${package_name}" "$(pkg-config --modversion "${package_name}")"
done

for disabled_library in libavdevice libavfilter; do
